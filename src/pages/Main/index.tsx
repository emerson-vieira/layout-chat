import React, { useState, useRef, useEffect } from 'react';

import SideMenu from '../../componets/SideMenu';
import SideMenuRight from '../../componets/SideMenuRight';
import LeftSidebar from '../../componets/LeftSidebar';
import Channel from '../../componets/Channel';

import Avatar from '../../componets/Avatar';

import { Container, Grid, BottomMenu, MessageIcon, GroupIcon, ContactsIcon, ButtonMenu } from './styles';

const Main: React.FC = () => {
    const optionsRef = useRef<HTMLDivElement>(null);
    const [menuSelected, setMenuSelected] = useState<string>('ChatList');

    useEffect(() => {
        if(optionsRef){
            const divActive = optionsRef.current.querySelector('.active');
            if(divActive.id === menuSelected) return;
            divActive?.classList.remove('active');  
            const id = `#${menuSelected}`;
            const divSelected = optionsRef.current.querySelector(id);
            divSelected.classList.add('active');
        }  
    }, [optionsRef, menuSelected]);

    function handleClickItemMenu(event: React.MouseEvent<HTMLButtonElement>){
        if(event.currentTarget.classList.contains('active')) return;
        setMenuSelected(event.currentTarget.id);
    }

    return (
        <Container>
            <Grid>
                <SideMenu subscribeMenuSelected={ () => setMenuSelected} menuSelected={menuSelected} />
                <LeftSidebar menuSelected={menuSelected} />
                <Channel />
                <SideMenuRight />
            </Grid>
            <BottomMenu ref={optionsRef}>
                <ButtonMenu className='active' onClick={handleClickItemMenu} id='ChatList'>
                    <MessageIcon/>
                </ButtonMenu>
                <ButtonMenu onClick={handleClickItemMenu} id='GroupList'>
                    <GroupIcon />
                </ButtonMenu>
                <ButtonMenu onClick={handleClickItemMenu} id='ContactsList'>
                    <ContactsIcon />
                </ButtonMenu>
                <ButtonMenu onClick={handleClickItemMenu}>
                    <Avatar url={'data:image/png;base64,'} />
                </ButtonMenu>
            </BottomMenu>
        </Container>
    );
}

export default Main;